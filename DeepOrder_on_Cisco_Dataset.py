...
# Reading the dataset
df = pd.read_csv('Datasets/Cisco.csv')
df.head()
values = pd.DataFrame()
values['Id'] =  range(0, 1)
values['env'] = 'Cisco'
...
def loss_function(information):
    ...
    save_figures(plt, 'loss_function_cisco')
    plt.close()
...
def actual_vs_prediction(y_test, y_test_pred):
    ...
    plt2.xlabel('Test Cases',fontsize=22)
    plt2.ylabel('Priority Values',fontsize=22)
    plt2.title("Comparision between 'Actual' and 'Predicted' values",fontsize=22)
    save_figures(plt2, 'actual_vs_prediction_cisco')
    plt2.close()

    plt3=plt
    plt3.plot(df_sorted['Actual'].tolist(), label='Actual')
    plt3.plot(df_sorted['Predicted'].tolist(), label='prediction')
    plt3.xlabel('Test cases',fontsize=22)
    plt3.ylabel('Priority Values',fontsize=22)
    plt3.title("Comparision between 'Actual' and 'Predicted' values")
    plt3.grid(which='major', linestyle='-', linewidth = '0.5', color='green')
    plt3.grid(which='minor', linestyle=':', linewidth = '0.5', color='black')
    plt3.legend()
    save_figures(plt3, 'actual_vs_prediction_2_cisco')
    plt3.close()
...
def save_figures(fig, filename):
    ...
    fig.savefig(os.path.join(FIGURE_DIR+'/Cisco', filename + '.pdf'), bbox_inches='tight')
...
def regression_line(y_test, y_test_pred):
    ...
    plt4.title("Neural Network Regression Line",fontsize=22)
    plt4.grid(which='major', linestyle='-', linewidth = '0.5', color='green')
    plt4.grid(which='minor', linestyle=':', linewidth = '0.5', color='black')
    save_figures(plt4, 'regression_line_cisco')
    plt4.close()
...
model.add(Dense(1,))
model.compile(Adam(lr=0.001), loss=root_mean_squared_error, metrics=[soft_acc])    

information = model.fit(X_train, y_train, validation_data= (X_test, y_test), epochs = 250
...
with open("cisco_model.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("cisco_model.h5")
print("Saved model to disk")
  
# load json and create model
json_file = open('cisco_model.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
# load weights into new model
loaded_model.load_weights("cisco_model.h5")
print("Loaded model from disk")
'''

#--------------------------------------------------------------#

values['MSE_list'] = str(mean(MSE_list))
...
# ## Calculating NAPFD on whole dataset
data_scaler = MinMaxScaler()
...
Cycle =   range(1, length)
  
plt5=plt  
plt5.plot(Cycle, NAPFD, color='red', marker='o')
plt5.title('NAPFD over cycles Cisco', fontsize=14)
plt5.xlabel('Cycles', fontsize=24)
plt5.ylabel('NAPFD', fontsize=24)
plt5.grid(True)
plt5.gcf().set_size_inches(20, 10)
save_figures(plt5, 'NAPFD_per_cycle_cisco')
plt5.close()
...
save_figures(plt, 'Prioritized_test_cases_class_distribution_cisco')
plt.close()
# Plotting Non-Prioritized test cases class distribution
sns.FacetGrid(df, hue="Verdict", height=6) \
   .map(sns.kdeplot, "Id") 
plt.legend(title='Verdict', loc='upper right', labels=['Passed', 'Failed'])
save_figures(plt, 'Non_Prioritized_test_cases_class_distribution_cisco')
plt.close()
...
save_figures(plt, 'binary_visualization_verdict_cisco')
plt.close()
...
plt.gcf().set_size_inches(20, 10)
save_figures(plt, 'Test_execution_time_overall_cisco')
plt.close()
...
plt.title('Frequency of Failed test cases in cisco', fontsize=14)
...
save_figures(plt, 'frequency_of_fault_detection_per_interval_cisco')
plt.close()
...
plt.legend()
plt.title('time over test cases cisco', fontsize=14)
plt.xlabel('test cases', fontsize=18)
plt.ylabel('time (average per interval))', fontsize=18)
plt.grid(True)
plt.legend(fontsize=16)
save_figures(plt, 'average_time_of_all_tests_per_interval_cisco')
plt.close()
...
plt.gcf().set_size_inches(15, 8)
save_figures(plt, 'total_time_of_all_tests_per_interval_cisco')
plt.close()
